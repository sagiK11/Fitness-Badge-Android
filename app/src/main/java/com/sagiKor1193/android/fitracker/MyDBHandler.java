...
public class MyDBHandler extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String query = "CREATE TABLE " + TABLE_DATA + " ( " +
                ...
                COLUMN_SCLASS + " TEXT NOT NULL, " + COLUMN_SAEROBIC_RES + " DOUBLE, "+ COLUMN_SAEROBIC_SCORE + " DOUBLE, "
                ...
                COLUMN_TOTAL_SCORE_WITHOUT_AEROBIC + " DOUBLE, "+ COLUMN_PHONE_NUMBER + " TEXT, "+
    ...
    public boolean addStudentData(Student data){
        ...
        values.put( COLUMN_SNAME , data.getSName());
        values.put( COLUMN_SCLASS , data.getSClass());
        values.put( COLUMN_PHONE_NUMBER , data.getPhoneNumber() );

        values.put( COLUMN_SAEROBIC_RES ,data.getAeroRes());
        values.put( COLUMN_SAEROBIC_SCORE , data.getAeroScore());
    ...
    public void deleteStudentData(String sName, String sClass){
        SQLiteDatabase db = getWritableDatabase();
    ...
    public Student getCurrentRow(int position){
        ...
        if (recordSet.getString(recordSet.getColumnIndex(COLUMN_SNAME)) != null) {
            newStudent.setSName(recordSet.getString(recordSet.getColumnIndex(COLUMN_SNAME)));
        }//
        if (recordSet.getString(recordSet.getColumnIndex(COLUMN_SCLASS)) != null) {
            newStudent.setSClass(recordSet.getString(recordSet.getColumnIndex(COLUMN_SCLASS)));
        }//2
        if (recordSet.getString(recordSet.getColumnIndex(COLUMN_SAEROBIC_RES)) != null) {
            newStudent.setAeroRes(Double.parseDouble(recordSet.getString(recordSet.getColumnIndex(COLUMN_SAEROBIC_RES))));
        }//3
        if (recordSet.getString(recordSet.getColumnIndex(COLUMN_SAEROBIC_SCORE)) != null) {
            newStudent.setAeroScore(Double.parseDouble(recordSet.getString(recordSet.getColumnIndex(COLUMN_SAEROBIC_SCORE))));
        }//2
        if (recordSet.getString(recordSet.getColumnIndex(COLUMN_SCUBES_RES)) != null) {
            newStudent.setCubesRes(Double.parseDouble(recordSet.getString(recordSet.getColumnIndex(COLUMN_SCUBES_RES))));
        }//4
        if (recordSet.getString(recordSet.getColumnIndex(COLUMN_SCUBES_SCORE)) != null) {
            newStudent.setCubesScore(Double.parseDouble(recordSet.getString(recordSet.getColumnIndex(COLUMN_SCUBES_SCORE))));
        }//5
        if (recordSet.getString(recordSet.getColumnIndex(COLUMN_SABS_RES)) != null) {
            newStudent.setAbsRes(Double.parseDouble(recordSet.getString(recordSet.getColumnIndex(COLUMN_SABS_RES))));
        }//6
        if (recordSet.getString(recordSet.getColumnIndex(COLUMN_SABS_SCORE)) != null) {
            newStudent.setAbsScore(Double.parseDouble(recordSet.getString(recordSet.getColumnIndex(COLUMN_SABS_SCORE))));
        }//7
        if (recordSet.getString(recordSet.getColumnIndex(COLUMN_HANDS_RES)) != null) {
            newStudent.setHandsRes(Double.parseDouble(recordSet.getString(recordSet.getColumnIndex(COLUMN_HANDS_RES))));
        }//8
        if (recordSet.getString(recordSet.getColumnIndex(COLUMN_HANDS_SCORE)) != null) {
            newStudent.setHandsScore(Double.parseDouble(recordSet.getString(recordSet.getColumnIndex(COLUMN_HANDS_SCORE))));
        }//9
        if (recordSet.getString(recordSet.getColumnIndex(COLUMN_JUMP_RES)) != null) {
            newStudent.setJumpRes(Double.parseDouble(recordSet.getString(recordSet.getColumnIndex(COLUMN_JUMP_RES))));
        }//10
        if (recordSet.getString(recordSet.getColumnIndex(COLUMN_JUMP_SCORE)) != null) {
    ...
    public  void updateStudent(String sName, String sClass, double aeroScore,int aeroRes, double cubesScore,
                               int cubesRes, double absScore, int absRes, double jumpScore, int jumpRes,
                               double handsScore, int handsRes, double totScore, String phoneNumber,
                               double avgWithOutAero){
        SQLiteDatabase db = getWritableDatabase();

        ContentValues content = new ContentValues();
        content.put(COLUMN_SAEROBIC_SCORE, aeroScore);
        content.put(COLUMN_SAEROBIC_RES, aeroRes);

        content.put(COLUMN_HANDS_SCORE, handsScore);
        content.put(COLUMN_HANDS_RES, handsRes);

        content.put(COLUMN_JUMP_SCORE , jumpScore);
        content.put(COLUMN_JUMP_RES, jumpRes);

        content.put(COLUMN_SABS_SCORE, absScore);
        content.put(COLUMN_SABS_RES, absRes);

        content.put(COLUMN_SCUBES_SCORE, cubesScore);
        content.put(COLUMN_SCUBES_RES, cubesRes);

        content.put(COLUMN_TOTAL_SCORE, totScore);
        content.put(COLUMN_TOTAL_SCORE_WITHOUT_AEROBIC , avgWithOutAero);

        content.put(COLUMN_PHONE_NUMBER, phoneNumber);

        String where =  COLUMN_SNAME + " = " + "\'"+ sName +   "\'"+ " and "+
                COLUMN_SCLASS + " = " +   "\'" + sClass + "\'";

        db.update(TABLE_DATA , content, where, null );
        db.close();
        Log.d(TAG,"Student phone " + phoneNumber);

    }

    public  void updateStudent(String sName, String sClass, double aeroScore, double cubesScore,
                                     double absScore, double jumpScore, double handsScore,int[] ctrl){
        SQLiteDatabase db = getWritableDatabase();
        ...
        if(db.update(TABLE_DATA , content, where, null ) != 1)
            ctrl[0] = 0;
