...
public class StudentActivity extends AppCompatActivity implements StudentActivityContract.View {
    ...
    protected void addScoresTextChangedListeners() {
        addScoreListener(etAerobicScore, SportResults.AEROBIC);
        addScoreListener(etCubesScore, SportResults.CUBES);
        addScoreListener(etHandsScore, SportResults.HANDS);
        addScoreListener(etJumpScore, SportResults.JUMP);
        addScoreListener(etAbsScore, SportResults.ABS);
    }

    private void addScoreListener(EditText editText, String type) {
        editText.addTextChangedListener((StudentTextWatcher)
                (charSequence, start, count, after) -> {
                    ...
                    presenter.onScoreTextChanged(input, type, map);
                }
        );
    }

    @Override
    public void popGenderError() {
